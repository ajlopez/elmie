...
function Parser(text, ctx) {
	...
	this.parseExpression = function () {
		...
		for (var token = lexer.nextToken(); token != null && token.type !== TokenType.Symbol && token.type !== TokenType.EndOfExpression; token = lexer.nextToken()) {
			...
			newexpr = parseBinaryExpression(0);
	...
	function parseBinaryExpression(level) {
		...
		while (token && token.type === TokenType.Symbol) {
			if (level === 0) {
				if (token.value === '+')
					expr = exprs.add(expr, parseBinaryExpression(level + 1));
				else if (token.value === '-')
					expr = exprs.subtract(expr, parseBinaryExpression(level + 1));
				else if (token.value === '=') {
				...
				if (token.value === '*')
					expr = exprs.multiply(expr, parseBinaryExpression(level + 1));
				else if (token.value === '/')
					expr = exprs.divide(expr, parseBinaryExpression(level + 1));
				else if (token.value === '//')
					expr = exprs.integerDivide(expr, parseBinaryExpression(level + 1));
				else
					break;
			}
			else
